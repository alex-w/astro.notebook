...
Расстояние $r$, с которого большая полуось орбиты Земли $a_\oplus$ видна под углом $\pi = 1''$ называется \term{1 парсеком}. Так как
\begin{equation}
...
\begin{figure}[h!]
	\begin{minipage}[b]{0.5\tw}
	   \centering
        \begin{tikzpicture}
            ...
            \tkzDefPoint(0,0){C}
            \tkzDefPoint(4,0){O}
            \tkzDefPointBy[homothety=center C ratio .3](O) \tkzGetPoint{U}
            ...
            \tkzLabelAngle[pos=1.4](I2,O,C){$\sfrac{\rho}{2}$}
            \tkzLabelAngle[pos=1.4](C,O,I1){$\sfrac{\rho}{2}$}
	...
	\begin{minipage}[b]{0.5\tw}
		...
		\begin{tikzpicture}
            ...
            \tkzDefPoint(0,0){O}
            \tkzDefPoint(4,0){S}
            \tkzDefPointBy[homothety=center O ratio .3](S) \tkzGetPoint{X}
            ...
            \tkzMarkAngle[size=.8](O,S,C)

            
            \tkzLabelSegment[below](C,S){$r$}
            \tkzLabelSegment[left=-2pt](C,O){$R_\oplus$}
            \tkzLabelAngle[pos=1.1, font=\footnotesize](O,S,C){$p$}
