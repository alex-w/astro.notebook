...
\begin{wrapfigure}[8]{r}{0.45\tw}
	...
	\begin{tikzpicture}
	   ...
	   \tkzDrawSegment[dim={{\footnotesize$p$},{-\xmax*0.45},left=1mm, midway}](F,D)
	   \tkzDrawSegment[dim={{\footnotesize$q$},{\xmax*0.9},right=1mm, midway}](F,O)
	   \tkzDrawSegment[dim={{\footnotesize$q$},{\xmax*0.9},right=1mm, midway}](O,D)
	   
	   \tkzLabelPoint[above right](F){$\mathbf{F}$}
	   \tkzLabelPoint[below right](O){$\mathbf{O}$}
	   \tkzLabelPoint[below](X){\adjustbox{left=6mm,raise=1ex}{$\mathbf{X}$}}
	   \tkzLabelSegment[above](F,P){$p$}
	   \tkzLabelSegment[left](F,X){$r$}
	   \tkzLabelSegment[left](X,X'){$d$}
