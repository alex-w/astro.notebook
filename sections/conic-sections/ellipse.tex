...
\begin{wrapfigure}[10]{r}{0.5\tw}
	...
	\centering
	 \begin{tikzpicture}
	   ...
	   \tkzDrawSegments[thick](Q1,Q2 F1,P P,F2 B1,B2 F1,B1 B1,F2)
	   
	   \tkzLabelPoint[below left](F1){$\mathbf{F_1}$}
	   \tkzLabelPoint[below right](F2){$\mathbf{F_2}$}
	   \tkzLabelPoint[below left](O){$\mathbf{O}$}
	   
	   \tkzLabelSegment[above](F1,P){$2a-p$}
	   \tkzLabelSegment[above](F1,B1){$a$}
	   \tkzLabelSegment[above](F2,B1){$a$}
	   
	   \def\k{1.05}
	   
	   \tkzDrawSegment[dim={$p$,{-(1-\e)/(1 - \e*\e)*\k*\k*\k},right=1mm, midway}](F2,P)
	   
	    \tkzDrawSegment[dim={$b$,{1/sqrt(1 - \e*\e)*\k},left=1mm, midway}](O,B2)
	    \tkzDrawSegment[dim={$b$,{-1/sqrt(1 - \e*\e)*\k},left=1mm, midway}](O,B1)

	   \tkzDrawSegment[dim={$a$,{-sqrt(1 - \e*\e)*\k},below=1mm, midway}](O,Q2)
	   \tkzDrawSegment[dim={$c$,{sqrt(1 - \e*\e)*\k / \e},below=1mm, midway}](O,F1)
	   \tkzDrawSegment[dim={$Q$,{-sqrt(1-\e*\e)*\k*\k*\k*\k / (1+\e)},below=1mm, midway}](F1,Q2)
	   \tkzDrawSegment[dim={$q$,{sqrt(1-\e*\e)*\k*\k*\k*\k / (1-\e)},below=1mm, midway}](F1,Q1)
	   
	   \tkzDrawPoints(O,F1,F2,Q1,Q2,B1,B2,P)     
